...
class CircuitLibraryCircuits():
    ...
    def integer_comparator(self, qubit, repeats, seed=100):
        if qubit < 2:
    ...
    def weighted_adder(self, qubit, repeats, seed=100):
        if qubit > 20:
    ...
    def graph_state(self, qubit, repeats, seed=100):
    ...
    def hidden_linear_function(self, qubit, repeats, seed=100):
        a = np.reshape([0] * (qubit ** 2), [qubit] * 2)
        for _ in range(qubit):
            while True:
                i = np.random.randint(0, qubit)
    ...
    def iqp(self, qubit, repeats, seed=100):
    ...
    def phase_estimation(self, qubit, repeats, seed=100):
        if qubit < 5:
            raise ValueError('qubit is too small: {0}'.format(qubit))
        return self._repeat(PhaseEstimation(2, QuantumVolume(qubit - 2).decompose()).decompose(), repeats)

    def pauli_feature_map(self, qubit, repeats, seed=100):
        return self._repeat(PauliFeatureMap(qubit, reps=1, paulis=['Z', 'YY', 'ZXZ']), repeats)

    def grover_search(self, qubit, repeats, seed=100):
        oracle = QuantumCircuit(qubit)
        oracle.z(0)
        return self._repeat(GroverOperator(oracle), repeats)

    def random_circuit(self, qubit, repeats, seed=100):
        return self._repeat(random_circuit(qubit, 10*qubit, max_operands=2, measure=True, seed=seed), repeats)

    def random_circuit_20(self, qubit, repeats, seed=100):
        return self._repeat(random_circuit(qubit, 20*qubit, max_operands=2, measure=True, seed=seed), repeats)

    def random_circuit_30(self, qubit, repeats, seed=100):
        return self._repeat(random_circuit(qubit, 30*qubit, max_operands=2, measure=True, seed=seed), repeats)
