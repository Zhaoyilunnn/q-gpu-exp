...
class SimulatorBenchmarkSuite(CircuitLibraryCircuits):
    ...
    DEFAULT_FUSION_BITS = 5
    DEFAULT_FUSION = False
    ...
    def gen_qobj(self, runtime, app, measure, measure_count, qubit):
        ...
        if app.endswith(".qasm"):
            circuit = QuantumCircuit().from_qasm_file(app)
        else:
            circuit = eval('self.{0}'.format(app))(qubit, None if app not in self.app2rep else self.app2rep[app], seed=self.seed)
        ...
        if self.save_qasm:
            if self.fusion:
                circuit.qasm(True, "%s-%s-fusion.qasm" % (app, qubit))
            else:
                circuit.qasm(True, "%s-%s.qasm" % (app, qubit))
            sys.exit(1)
        #print(circuit)
    ...
    def _run(self, runtime, app, measure, measure_count, noise_name, qubit):
        ...
        if self.save_conf:
            with open(("%s_%s_circ.job" % (app, qubit)), 'w') as job:
                job.write(json.dumps(qobj.to_dict()))
            with open(("%s_%s_circ.conf" % (app, qubit)), 'w') as conf:
                conf.write(json.dumps(backend_options))
